...
export function parseSearch(html: string, options: SearchOptions): Promise<(Video|Playlist|Channel)[]> {
...
export function parseGetPlaylist(html: string): Promise<PlaylistDetailed | {}> {
...
export function parseGetVideo(html: string): Promise<VideoDetailed | {}> {
		...
		try {
			...
			const video = {
				...
				channel: {
					...
					thumbnail: 
					videoInfo.owner.videoOwnerRenderer.thumbnail.thumbnails[videoInfo.owner.videoOwnerRenderer.thumbnail.thumbnails.length - 1].url.startsWith("https:") ? 
...
export function parseGetRelated(html: string, limit: number): Promise<Video[]> {
	...
	return new Promise(function (resolve) {
		...
		return resolve(relatedVideos);
	});
}

export function parseGetUpNext(html: string): Promise<Video | {}> {
