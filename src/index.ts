 ...
 */
const getPlaylist = async (playlistId: string): Promise<PlaylistDetailed|{}> => {
 ...
 */
const getVideo = async (videoId: string): Promise<VideoDetailed|{}> => {
 ...
 */
const getRelated = async (videoId: string, limit = 10): Promise<Video[]> => {
	if (videoId.trim().length === 0) throw(new Error("Video ID cannot be blank"));
	const videoUrl = `${url}watch?v=${videoId}`;
	const html = await request(videoUrl);
 ...
 * @param videoId Id of the video
 */
const getUpNext = async (videoId: string): Promise<Video|{}> => {
	if (videoId.trim().length === 0) throw(new Error("Video ID cannot be blank"));
	const videoUrl = `${url}watch?v=${videoId}`;
	const html = await request(videoUrl);
